...
# init parameters in remote machine
variables = {
...
def serial_command_generator(content):
    global COLOR_DOWN_RED, COLOR_UP_GREEN, variables
    # print('got it')
    parameter = variables[content['currency']]
    commands = []
...
def get_price_change():
    global eth_price_change, btc_price_change
...
def on_message(wsapp, message):
    ...
    if 'ping' in result:
        ...
        if result['ch'] == 'market.ethusdt.detail':
            currency = 'ETH'
            change = eth_price_change
        if result['ch'] == 'market.btcusdt.detail':
            currency = 'BTC'
            change = btc_price_change
        
        # print(currency, result['tick']['close'], 0 if float(change)<0 else 1,float(change), result['tick']['high'], result['tick']['low'])
